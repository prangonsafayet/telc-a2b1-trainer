// Node validator for exam data files. Usage: node validate.js [data/examNN.js ...]
// With no args, validates every data/exam*.js file.
const fs = require('fs');
...
const files = process.argv.slice(2).length
  ...
  : fs.readdirSync(path.join(__dirname, 'data')).filter(f => /^exam\d+\.js$/.test(f)).sort().map(f => 'data/' + f);
...
for (const file of files) {
  ...
  const exams = (ctx.window.TELC_EXAMS || []);
  if (exams.length !== 1) { err(file, `expected exactly 1 exam pushed, got ${exams.length}`); continue; }
  const ex = exams[0];
  const f = file;
