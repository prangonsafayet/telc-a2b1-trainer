...
export const singleLevelModuleBriefing = (module: ExamModule, level: SingleLevelTrainerId): string => {
  switch (module) {
    ...
    case 'sprachbausteine':
      return '2 parts, 20 items, 30 points. Teil 1: a letter with 10 grammar gaps — pick a, b or c (1.5 pts each). Teil 2: a letter with 10 gaps — fill them from a bank of 15 words, each word fits only once (1.5 pts each). Look left AND right of every gap.';
    case 'hoeren':
      return level === 'b1'
